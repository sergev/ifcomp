...
CompareResult hashcode_compare(const HashInfo& ha, const HashInfo& hb) {
    if (ha.h1 < hb.h1) return lt;
    if (ha.h1 > hb.h1) return gt;
    if (ha.h2 < hb.h2) return lt;
    if (ha.h2 > hb.h2) return gt;
    return eq;
}
...
void enter_line(const std::string& text, const HashInfo& h, line_count linen, int input_file,
    ...
    while (current_node != null_hash_list) {
        ...
        if (test == eq) {
            ...
            string_table[last_SI].next_text_with_same_hash = SI = setup_distinct_text(text, linen, input_file);
            result_hash_node = current_node;
            result_string_index = SI;
            return;
        }
    ...
    hash_node[last_node].next_in_bucket = current_node = setup_hash_node(SI, text, linen, input_file, h);
    result_hash_node = current_node;
    result_string_index = SI;
}
...
void print_hash_node(const HashNodeDecl& p) {
    std::printf("h2=%lx  h1=%x  text_list=%d  nextb=%d\n", 
                static_cast<unsigned long>(p.h.h2), p.h.h1, p.text_list, p.next_in_bucket);
}

void print_string(const StringDecl& p) {
    std::printf("| %s | nexth=%d f1l=%d f2l=%d f1lst=%d f2lst=%d\n", 
                p.text.c_str(), p.next_text_with_same_hash, 
                p.file_nlines[first_file], p.file_nlines[second_file],
