...
void print_statistics() {
    ...
    std::printf("%8zu (%zu max, %u bytes) %s entries used.\n", 
                string_table.size(), string_table.capacity(), msize, "string");
    mem_used += msize;
    
    // line_table
    msize = static_cast<unsigned>(line_table.size() * sizeof(LineTableDecl));
    std::printf("%8zu (%zu max, %u bytes) %s entries used.\n", 
                line_table.size(), line_table.capacity(), msize, "line_table");
    mem_used += msize;
    
    // file_line[first_file]
    msize = static_cast<unsigned>(file_line[first_file].size() * sizeof(FileLineDecl));
    std::printf("%8zu (%zu max, %u bytes) %s entries used.\n", 
                file_line[first_file].size(), file_line[first_file].capacity(), msize, "file_line[first_file]");
    mem_used += msize;
    
    // file_line[second_file]
    msize = static_cast<unsigned>(file_line[second_file].size() * sizeof(FileLineDecl));
    std::printf("%8zu (%zu max, %u bytes) %s entries used.\n", 
                file_line[second_file].size(), file_line[second_file].capacity(), msize, "file_line[second_file]");
    mem_used += msize;
    ...
    std::printf("%8zu (%zu max, %u bytes) %s entries used.\n", 
                node.size(), node.capacity(), msize, "node");
    mem_used += msize;
