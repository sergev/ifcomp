...
typedef struct {unsigned short h1; long h2;} hash_info;
typedef enum {False, True} bool;
bool debug_syt_full = False, debug_syt = False,
     debug_dump_trees = False, debug_dump_trees_full = False,
     debug_alloc = False, statistics = False, free_ok = True;
...
#define array_of(name) name##_decl *name; int last_##name, hbound_##name;
#define array_of1(name,w) name##_decl *name w; int last_##name w, hbound_##name w;

// Cells for linked list of line numbers.

#define null_line_list 0
typedef struct { line_count linen, next; } line_table_decl;
array_of(line_table);
...
typedef struct {
    ...
    line_count linen; line_type ptr_type;
    } file_line_decl;
...
void print_string(string_decl *p) {
    printf("| %s | nexth=%d f1l=%d f2l=%d f1lst=%d f2lst=%d\n",
        p->text, p->next_text_with_same_hash, p->file_nlines[first_file],
        p->file_nlines[second_file],
...
void dump_syt(hash_node_index node) {        // 4366
...
void format_file_line(file_line_decl *p) {
    printf("|%3d|",p->linen);
    switch(p->ptr_type) {
        case syt_type: printf("S     "); break;
        case unique_type: printf("U%5d",p->ptr0); break;
        case match_type: printf("M%5d",p->ptr0); break;
...
void test_list(int pass) {	// 4634
	...
	if (j > total_file_nlines[first_file]) printf("=============\n");
	else format_file_line(file1_line+j);
	if (j <= total_file_nlines[second_file]) format_file_line(file2_line+j);
	}
    CR();
    }
...
int next_index_func(int *last, int *hbound, char *name, int size, void **p) {
	...
	if (new_size > cur_size) {
		...
		*p = newp;
	    ...
	    else printf("Realloc returned 0.\n");
	    }
	else printf("New size shrank!:%u\n",new_size);
	}
...
#define next_index(name) \
	next_index_func(&last_##name,&hbound_##name,#name,\
...
PUBLIC void * Mmalloc(size_t Amount) {
    if (Amount == 0) Amount = 1;        // 0 fails on aix 386.
...
void enter_line(char *text, hash_info h, line_count linen,
	int input_file,
...
if (debug_syt_full)
	...
	S.file_nlines[input_file] = 1; S.file_nlines[other] = 0;
...
//  	S.nunique_lines = 1;
	...
	*hash_start_node = node = setup_hash_node(&SI);
	...
	if (test == eq) {
	    ...
	    // exists already.
	    for (SI = hash_node[node].text_list;
	         SI != null_string_list;
	         last_SI = SI,
	         SI = string[SI].next_text_with_same_hash) {
	        if (strcmp(string[SI].text,text) == 0) {
	            add_linen_to_text_list(SI);
	            goto finish;
	            }
                }
            string[last_SI].next_text_with_same_hash =
		SI = setup_distinct_text();
            goto finish;
            }
        if (test == lt) {
...
if (N < 0 || N > hbound_hash_node) printf("OOPS! node=%d &=%x\n",N,&hash_node[node].next_in_bucket);
        node = hash_node[node].next_in_bucket;
        }
    // Add to chain.
    if (last_node == null_hash_list) printf("?OOPS empty list!\n");
    hash_node[last_node].next_in_bucket = node = setup_hash_node(&SI);
finish: ;
    *result_hash_node = node; *result_string_index = SI;
    }
...
void read_lines(int which_file) {
    ...
    while (1) {
        ...
        if (*buf == 0 && feof(input_file[which_file])) break;
        int len = strlen(buf);
        if (len && buf[len-1] == '\n') buf[len-1] = 0;
...
// Pass1.
void pass1() {
    read_lines(first_file); read_lines(second_file);
...
void pass2() {
	...
	if (string[i].file_nlines[first_file] == 1 &&
	    string[i].file_nlines[second_file] == 1) {
	    // Found a unique pair.
...
void pass3() {
	...
	if (file1_line[m].ptr_type == unique_type) {
	    ...
	    for (m++, n++;
	         file1_line[m].ptr_type == syt_type &&
...
typedef struct {tree_index start, end; } tree_bounds;
...
tree_index allocate_node() {
    if (free_nodes_start == null_node) return next_index(node);
    else {
...
#define get_start_finish(noden) \
    ...
    if (!leaf(noden)) start = node[noden].branch_start, finish = noden; \
    else start = noden, finish = node[noden].next;
...
void format_node(tree_index noden, int pad) {
    ...
    printf("[%d<-N%d->%d, cost=%2d linen=%2d",
        X(prev),noden, X(next), X(cost), X(linen));
    line_count L = X(linen); int fileno;
...
tree_index make_node(node_decl *p) {
    ...
    node[i] = *p;
    if (debug_dump_trees_full) printf("just made "), format_node(i,0);
    return i;
    }

void dump_tree(tree_index tree_start) {
    printf("Tree %d:\n",tree_start);
    bool branch = False; tree_index T = tree_start;
    while (T != null_node) {
	tree_index T2 = T;
	if leaf(T) {
	    format_node(T, branch ? 8 : 1), T = node[T].next;
	    if (debug_dump_trees_full) print_node1(T2,True,0);
	    }
	else {
	    if (branch) branch = False, T = node[T].next;
	    else format_node(T,1), T = node[T].branch_start, branch = True;
	    }
	}
    }
...
void dump_trees(int pass) {
    if (!debug_dump_trees) return;
    printf(pass == no_pass ? "dump trees\n" : "dump_trees after pass%d\n",pass);
    dump_tree(tree1_start); dump_tree(tree2_start);
    }

void pass5() {	// 70e0
    void doit(int fileno, node_decl *Np) {
	if (debug_dump_trees) printf("Make tree for file %d\n",fileno+1);
	...
	// There are also a header and trailer node for each file.
		...
		while (fp[i+1].ptr_type == syt_type &&
		       i+1 <= total_file_nlines[fileno]) i++;
	        i++;
		Np->cost = i-Np->linen;
		...
		while (fp[i+1].ptr_type != syt_type &&
		       fp[i+1].ptr0 == exp_ptr0 &&
		       i+1 <= total_file_nlines[fileno]) i++, exp_ptr0++;
		i++;
		Np->cost = i-Np->linen;
		}
	    if (fileno == second_file) Np->linen = - Np->linen;
...
tree_index find_node(tree_bounds T, tree_index linen) { // 5eb0
    linen = _abs(linen);
    ...
    N = T.start; printf("[");
    while (N != T.end) printf("%d ",N), N = node[N].next;
    printf("] ln=%d\n",linen);
    internal_error("find_node"," sn=%d en=%d l=%d",T.start,T.end,linen);
    }

void detach_node(tree_index noden) { // 5f20
...
void combine_nodes(tree_index node1, tree_index node2) {  // 5fd8
    ...
    detach_node(node2);
    N.prev = node[node1].prev; N.next = node[node1].next;
    ...
    else N.branch_start = branch_link1 = node1;
    if (!leaf(node2)) {
    ...
    else branch_link2 = N.branch_end = node2;
...
void ph(char *s, char dash, bool space) {
    ...
    printf("*** %s",s);
    int pad = 52;
    if (space) printf(" "); else pad++;
    for (int i = 0; i < pad-len; i++) printf("%c",dash);
    printf(" ***\n");
    }
...
void after_lines(tree_index noden) { // 6320
    tree_index unique_find(tree_index noden) {        // 5de0
	...
	for (line_count start_line = end_line + node[noden].cost-1;
		start_line >= end_line;
		start_line--)
...
loop1: ;
	...
	if (leaf(start)) {
	    ...
	    if (linen != null_node) break;
	    linen = 0; last = start; start = node[start].prev;
	    goto loop1;
	    }
	if (last == node[start].branch_start)
	    start = node[start].prev, last = start;
	else last = start, start = node[start].branch_end;
	}
...
loop2 : ;
	...
	if (last == node[start].branch_end)
	    last = start, start = node[start].next;
	else last = start, start = node[start].branch_start;
	}
    }
...
#define top_msg() print_header("AFTER TOP")
	top_msg();
    else after_lines(noden);
    }

void delete_lines(tree_index noden) {	// 6626
    nchange_blocks++;
...
void pass6() {	// 7508
    tree_index replaceable(tree_index noden) {	// 6a18
	// Replaceable if :
    ...
    void replace_lines(tree_index node1, tree_index node2) {	// 6b08
	nchange_blocks++;
	...
	if (prev == tree1_start) {
	    detach_node(node2);
	    node[tree1_start].branch_start =
	    node[tree1_start].branch_end = node2;
	    node[node2].prev = node[node2].next = tree1_start;
	    }
	else combine_nodes(prev,node2);
	dump_trees(no_pass);
	}

    void insert_lines(tree_index noden) {	// 6dea
	nchange_blocks++;
	...
	if (i == tree2_start) {
	    detach_node(noden);
	    node[tree1_start].branch_start =
	    node[tree1_start].branch_end = noden;
...
#define insert_msg() print_header1("INSERT LINE(s)")
	...
	while (i != tree1_end) {
		...
		if (location_in_other_file == null_node) delete_lines(i);
		else replace_lines(i,location_in_other_file);
		}
	    i = j;
	    }
	}
...
void pass7() {	// 75ee
    ...
    bool combine_adjacent_nodes(tree_index node1) {	// 654c
	...
	if (debug_dump_trees_full)
		...
		node[node1].linen, node2, node[node2].linen);
...
void pass8() {	// 7678
    void move_lines(tree_index node1, tree_index node2) { // 679a
	nchange_blocks++;
...
#define move_msg() print_header1("MOVE LINE(s)");
    ...
    tree_index min_cost_node(tree_index start_node, tree_index end_node) { // 5f6a
	...
	while (N != end_node) {
        ...
        if (debug_dump_trees_full) printf("min_cost_node(%d,%d)=%d\n",
...
RETRY: ;
	...
	if (debug_dump_trees_full)
	    ...
	    i,true_line_of(i),file1_line[true_line_of(i)].ptr0,
	    j,true_line_of(j));
	while (file1_line[true_line_of(i)].ptr0 == true_line_of(j)
	  && i != tree1_end) i = node[i].next, j = node[j].next;
	if (i == tree1_end) return;
...
int sum_kind(line_kinds l) { return l.non_cosmetic + l.cosmetic; }
...
//  #define NFMAT "%d(%d cos, %d non-cos)"
    ...
    printf(NFMAT "lines deleted from old and replaced with %d "
...
#define alloc_thing(thing, number) \
	(thing = alloc(number*sizeof(*thing)), \
	last_##thing = 0, hbound_##thing = number)
#define alloc_thing1(thing, number,w) \
	(thing = alloc(number*sizeof(*thing)), \
	last_##thing##w = 0, hbound_##thing##w = number)
...
void doit() {
    for (int i = 0; i < nbuckets; i++) sec_hash_start_node[i] = null_hash_list;
    allocate_tables();
...
FILE *open_file(char *fn) {
    FILE*fp = fopen(fn,"r");
    extern int errno;
    if (fp == 0) printf("Can't open file %s:%s\n",fn,strerror(errno)), exit(1);
    return fp;
    }

void memory_statistics() {
    long mem_used = 0; unsigned msize;
    #define P(X) printf("%8d (%d max, %u bytes) %s entries used.\n", \
	last_##X,hbound_##X-1,msize=(hbound_##X-1)*sizeof(*X),#X), \
	mem_used += msize
    P(hash_node);
...
void main(int argc, char** argv) {
	...
	if (arg[0] == '-')
	    if (p_is("-stfull")) debug_syt_full = True;
	    else if (p_is("-st")) debug_syt = True;
	    else if (p_is("-trees")) debug_dump_trees = True;
	    else if (p_is("-treesfull")) debug_dump_trees_full = True;
	    else if (p_is("-alloc")) debug_alloc = True;
	    else if (p_is("-stat")) statistics = True;
	    else if (p_is("-nofree")) free_ok = False;
	    else if (p_is("-debug")) {
		debug_syt_full = True;
		debug_syt = True;
    ...
    if (fileno <= second_file) help();
    printf("Comparing:  %s %s\n\n",fnames[first_file],fnames[second_file]);
    doit();
    if (statistics) print_statistics();
    }
