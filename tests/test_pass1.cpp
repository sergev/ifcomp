...
TEST_F(Pass1, MakeLineEntry_Chain)
{
    ...
    EXPECT_EQ(ifc.line_table[entry3].linen, 3);
    EXPECT_EQ(ifc.line_table[entry3].next, entry2);
...
TEST_F(Pass1, SetupHashNode_Basic)
{
    ...
    EXPECT_GE(tip, 0);
    EXPECT_EQ(ifc.hash_node[node_idx].h, h);
...
TEST_F(Pass1, EnterLine_SameHashDifferentText)
{
    ...
    EXPECT_TRUE(ifc.string_table[si1].next_text_with_same_hash == si2 ||
                ifc.string_table[si2].next_text_with_same_hash == si1 ||
                ifc.hash_node[node1].text_list == si1 || ifc.hash_node[node1].text_list == si2);
}
...
TEST_F(Pass1, TwoIdenticalFiles)
{
    ...
    EXPECT_EQ(ifc.string_table[si2_a].text, "A");
